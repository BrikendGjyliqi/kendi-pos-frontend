...
/** Porositë e mbyllura (jo te paguara) per kete tavoline — read-only */
const closedOrders = computed<Order[]>(() => {
  if (!selectedTable.value) return []
  return ordersStore.closedOrdersForTable(selectedTable.value.id)
})

/** Total i krejt porosive te papaguara (open + closed) — kjo paguhet me Fiskalizo */
const grandTotal = computed(() => {
  if (!selectedTable.value) return 0
  return ordersStore.tableTotal(selectedTable.value.id)
})

/** A ka ndonje porosi qe mund te paguhet? (te pakten 1 closed me items, ose order aktiv me items) */
...
/** "Përfundo" — ruan porosinë dhe kthehet te tavolinat.
 * Porosia mbetet 'open' — mund te hyrësh prap dhe te shtosh me shume. */
/** "Përfundo" — mbyll porosine aktuale (closed), tavolina mbetet e zene me totalin.
 * User-i mund te hyje prap dhe te shtoje nje porosi te re. */
async function finishOrder() {
  if (currentOrderId.value) {
    ...
    if (!order) {
      ...
      // Mbyll porosine aktive (status: open → closed)
...
/** Hap nje porosi te re ne te njejten tavoline (mbyll te vjeteren me pare) */
...
async function addProduct(product: Product) {
  ...
  // Lazily create the open order the first time a product is added.
  // This way, opening a table doesn't pollute the DB with empty orders.
  if (!currentOrderId.value) {
    const order = await ordersStore.ensureOpenOrder(
      selectedTable.value.id,
      auth.currentStaff?.id
...
function openPayment() {
  ...
  // Para se te paguhet, mbyll porosine aktive (nese ka items) qe edhe ajo te paguhet bashke
...
const changeCents = computed(() => {
  ...
  return Math.max(0, cashGivenCents.value - grandTotal.value)
})

const canFinish = computed(() => {
  if (grandTotal.value === 0) return false
  if (paymentMethod.value === 'card') return true
  return cashGivenCents.value >= grandTotal.value
})
      ...
      <aside class="order">
        <header class="order-head">
          ...
          <button v-if="currentOrder && currentOrder.items.length > 0"
            class="new-order-btn" @click="startNewOrder" title="Filloj porosi të re">
            <Plus :size="14" />
            Porosi e re
          </button>
        </header>

        <!-- Closed orders (read-only) shown at top -->
        ...
        <!-- Active order header (only if there are closed orders too) -->
        ...
        <footer class="order-foot">
          ...
          <div class="action-row">
            ...
            <button class="action-btn action-btn--fiscal"
              ...
              @click="openPayment">
              Fiskalizo {{ closedOrders.length > 0 ? 'të gjitha' : '' }}
            </button>
          </div>
        </footer>
      </aside>
        ...
        <div class="products-wrap">
          <div v-if="filteredProducts.length > 0" class="products">
            <button v-for="product in filteredProducts" :key="product.id"
              class="product" @click="addProduct(product)">
              <span class="product-color"
                :style="{ background: catColor(product.categoryId) }"></span>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <section class="pay-left">
            <div class="pay-row">
              <span class="pay-label">Totali</span>
            ...
            <div class="pay-row pay-row--input">
              ...
              <span class="pay-val pay-val--input mono">
                {{ paymentMethod === 'card' ? formatMoney(grandTotal)
                  : formatMoney(cashGivenCents) }}
              </span>
            </div>

            <div class="pay-row pay-row--change">
              <span class="pay-label">Kthimi</span>
              <span class="pay-val pay-val--change mono">
